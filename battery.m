%the following code simulate the operations of an green ammonia synthesis process using a wind farm, battery park and hydrogen storage, since the sythesis process is powered by the grid
...
cf=E_tot/(8784*optimal_turbines*4);%capacity factor of the plant (4 MW turbine)
fprintf('the capacity factor of the plant is: %.2f \n', cf);
%% Hydrogen production requirement (converted to constant power load)
m_H2required = (6481.3 * 1000 / 8784); % Required hydrogen flow (kg/h)
...
fprintf('🔋 Minimum number of batteries required: %d\n', n_batteries);
...
% Initialization
SOC = zeros(8784, 1);                  % state of charge [kWh]
SOC(1) = 0.75 * battery_capacity_kWh;  % initial SOC  (75%)
total_cycles = zeros(8784,1);          % cumulative cycles
SoH_battery = zeros(8784,1);           % state of health
SoH_battery(1) = 100;                  
...
end_life = false;
battery_output_energy = zeros(8784,1); % energy erogated from the batteries(kWh)
...
for t = 2:8784
    ...
    if ~end_life && SoH_now < 70
        fprintf('⚠️ Battery at the end of life after %.1f cicli (anno: %.2f)\n', total_cycles(t), t/8784);
        end_life = true;
    end
end
E_battery_total_kWh = sum(battery_output_energy);%KWh
E_battery_lifetime=E_battery_total_kWh*20;%KWh
fprintf('🔋 Energia totale fornita dalla batteria durante l\''anno: %.2f kWh\n', E_battery_total_kWh);
...
% Initialisations 
H2_total = zeros(8784,1);
H2_from_wind = zeros(8784,1);
H2_from_batt = zeros(8784,1);
H2_reserve = zeros(8784,1);
...
for t = 1:8784
    ...
    % 2️⃣ Deficit → battery turns on to supply
    ...
    % 3️⃣ total hydrogen produced without taking or putting in/from storage
    H2_total(t) = H2_from_wind(t) + H2_from_batt(t);
    H2_reserve_prev = H2_reserve(max(t-1,1));

    % 4️⃣ extra production for storage (PEM extra fixed)
    ...
    % 5️⃣ if deficit of H₂  → use the storaged one (reserve)
    if H2_total(t) < m_H2required
        ...
        if H2_from_reserve > 0
           ...
           %fprintf("✅ Riserva usata in ora %d: %.2f kg\n", t, H2_from_reserve);
        end
    end
end
...
% final Debug info 
fprintf('\n⏳ Ore in deficit H₂: %d\n', count_deficit_hours);
fprintf('✅ Ore in cui è stata usata la riserva: %d\n', count_reserve_use);
fprintf('📤 Totale H₂ prelevato dalla riserva: %.2f kg\n', H2_from_reserve_used);
fprintf('📥 Totale H₂ accumulato nella riserva: %.2f kg\n', H2_extra_total);
fprintf('📦 Massima riserva H₂ raggiunta: %.2f kg\n', max(H2_reserve));
%%%FIXED COSTS 
%% ECONOMIC 
...
el_cost_pem=(LCOE_wf/1000)*(mean(eta/kWh_per_kg_H2))*(sum(H2_total)*8784);% eur/y cost of electricity for the production of hydrogen
...
E_c_tot=1.535227481997236e+07-(404.6583*8784)+(sum(P_O2comp*8769));%kWh
