...
for t = 1:length(P_generated)
    ...
    reserve1 = reserve(1:3000); % First 6552 hours (Jan to Sept)
...
Comprex_system_cost = 1169.3 - 897.6240 + Comprex_cost_O2;%KEUR 
N2_plant_cost = 3359.6;%KEUR
other_cost = 4093.6;%KEUR
wind_farm = CAPEX_turb / 1000;%KEUR
ammonia_loop = Reactor_cost + other_cost;
data = [Comprex_system_cost, Cost_PEM_scaled, Reactor_cost, N2_plant_cost, ammonia_loop, wind_farm, Hydrogen_stroage_cost];
percentuali = data / sum(data) * 100;

figure;
pie(data);
legend('Compression System Cost', 'Hydrogen System', 'Reactor Cost', 'Nitrogen', 'Ammonia Loop', 'Wind Farm', 'Hydrogen Storage', 'Location', 'southeastoutside');
...
REVENUE_NH3y=1014.8*m_NH3;%eur/y
